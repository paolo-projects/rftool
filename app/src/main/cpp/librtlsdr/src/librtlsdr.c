...
int rtlsdr_open(rtlsdr_dev_t **out_dev, int fileDescriptor)
{
	int r;
	int i;
	libusb_device **list;
	...
	r = libusb_set_option(&dev->ctx, LIBUSB_OPTION_NO_DEVICE_DISCOVERY);
    if(r < 0) {
        return r;
    }
	...
	r = libusb_wrap_sys_device(NULL, (intptr_t)fileDescriptor, &dev->devh);
